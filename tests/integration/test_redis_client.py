...
async def test_redis_item_features_repository(
    ...
    media_items_result = await cache_repository.get_item_features(media_item_collection_ids, ["name", "genres"])
...
async def test_redis_user_item_features_repository(
    ...
    user_item_features_result = await cache_repository.get_user_item_features(
        user_id, item_ids, ["is_like", "is_dislike"]
    )
...
async def test_redis_user_features_repository(
    ...
    result = await cache_repository.get_user_features(first_user.user_id, features=["username"])
    assert result.username == first_user.username

    first_user.is_viewed_series = True
    await cache_repository.save_user_features(first_user)
